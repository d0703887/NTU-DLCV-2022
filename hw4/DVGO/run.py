...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      ...
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False, filenames=None):
...
def load_everything(args, cfg):
    ...
    '''
    data_dict = load_data(cfg.data, args.test_json)
...
if __name__=='__main__':
    ...
    if args.render_test:
        ...
        filenames = []
        test_file = json.load(open(args.test_json, 'r'))
        for frame in test_file['frames']:
            filenames.append(frame['file_path'].split('/')[-1] + '.png')
        if args.output_path == '':
            testsavedir = os.path.join(cfg.basedir, cfg.expname, f'render_test_{ckpt_name}')
        else:
            testsavedir = args.output_path
