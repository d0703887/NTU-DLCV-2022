...
def load_data(args, test_json):
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, i_split = load_blender_data(args.datadir, args.half_res, args.testskip, test_json)
    ...
    # Add HW for missing images
    tmp = np.full((len(poses) - len(images), 2), HW[0])
    HW = np.concatenate((HW, tmp))
