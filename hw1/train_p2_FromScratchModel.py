...
class imageDataset(Dataset):
    ...
    def __getitem__(self, index):
        image = self.toTensor(
            transforms.Resize((256, 256))(Image.open(os.path.join(self.root, self.filenames[index] + '_sat.jpg'))))
        if self.mode == 'train':
...
# Nerual network(VGG16 + FCN32s)
class F32s(nn.Module):
    def __init__(self):
        super(F32s, self).__init__()

        self.vgg = torchvision.models.vgg16(weights=torchvision.models.VGG16_Weights.DEFAULT).features
        self.vgg[0] = nn.Conv2d(3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(100, 100))
        ...
        self.score = nn.Conv2d(4096, 7, kernel_size=1, stride=1, padding=0)
        self.up = nn.ConvTranspose2d(7, 7, kernel_size=64, stride=64, bias=False, padding=0)
...
def train(model, train_loader, val_loader, config, device, resume, path=None):
    criterion = nn.CrossEntropyLoss(weight=torch.tensor((0.3, 2.4, 7.2, 1.6, 2.4, 0.5, 1.8)).to(device))
...
# Model
model = F32s().to(device)


# training
train(model, train_loader, val_loader, config, device, True,
