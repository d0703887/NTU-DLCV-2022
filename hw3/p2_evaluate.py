...
class CLIPScore:
    ...
    def getCLIPScore(self, image, caption):
        ...
        """
        w = 2.5
        image = self.preprocess(image).to('cuda').unsqueeze(0)
        caption = clip.tokenize([caption]).to('cuda')
        with torch.no_grad():
            image = self.model.encode_image(image)
            caption = self.model.encode_text(caption)
        image /= image.norm(dim=-1, keepdim=True)
        caption /= caption.norm(dim=-1, keepdim=True)
        similarity = (image @ caption.T)
        return w * max(similarity[0][0], 0)
